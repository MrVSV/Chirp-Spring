package com.vsv.chirp.infra.rate_limiting

import com.vsv.chirp.infra.config.NginxConfig
...
class IpResolver(
    ...
    fun getClientIp(request: HttpServletRequest): String {
        ...
        if (!isFromTrustedProxy(remoteAddr)) {
            if (nginxConfig.requireProxy) {
                ...
                throw SecurityException("No valid security IP proxy headers")
            }
            return remoteAddr
        }
        ...
        if(clientIp == null) {
            logger.warn("No valid client IP proxy headers")
            if(nginxConfig.requireProxy) {
                throw SecurityException("No valid security IP proxy headers")
            }
        }
    ...
    private fun validateAndNormalizeIp(
        ...
        if (trimmedIp.isBlank() || INVALID_IPS.contains(trimmedIp)) {
            logger.debug("Invalid IP in $headerName: $ip from $proxyIp")
            return null
        }
        ...
        return try {
            ...
            logger.warn("Invalid IP format in $headerName: $ip from $proxyIp, $e")
            null
        }
    }
